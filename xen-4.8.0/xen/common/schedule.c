...
/* Added by KevinJio */
#include <xen/mctop.h>
extern mctop_t complete_topology;
/**
The following is the content of mctop_serialization.c
It is the implementation of (mctop_serialization.h) the functions declared in public/sched.h
*/
...
void serialize_mctop(mctop_t* mctop, char** dest) {
    ...
    // Reinitialize the hwcs_grps_array and siblings_array arrays
    n_hwcs_grps = 0;
    n_siblings = 0;
...
mctop_t* deserialize_mctop(mctop_t* mctop, char** src) {
    ...
    // mctop->hwcs = (hw_context_t*) malloc(mctop->n_hwcs * sizeof (hw_context_t));
    mctop->hwcs = xmalloc_array(hw_context_t, mctop->n_hwcs);
    for (i = 0; i < mctop->n_hwcs; i++) {
	...
	//printk("mctop->socket_level %d\n", mctop->socket_level);
	//printk("mctop->n_sockets %d\n", mctop->n_sockets);
	//printk("mctop->has_mem %d\n", mctop->has_mem);

    // mctop->sockets = (socket_t*) malloc(mctop->n_sockets * sizeof (socket_t));
    mctop->sockets = xmalloc_array(socket_t, mctop->n_sockets);
    for (i = 0; i < mctop->n_sockets; i++) {
    ...
    // mctop->siblings = (sibling_t**) malloc(mctop->n_siblings * sizeof (sibling_t*));
    mctop->siblings = xmalloc_array(sibling_t*, mctop->n_siblings);
    for (i = 0; i < mctop->n_siblings; i++) {
...
socket_t* deserialize_socket(socket_t* socket, char** src, uint has_mem) {
    ...
    *((int*) &socket->next) = id;
    ...
    // socket->hwcs = (hw_context_t**) malloc(socket->n_hwcs * sizeof (hw_context_t*));
    socket->hwcs = xmalloc_array(hw_context_t*, socket->n_hwcs);
    for (i = 0; i < socket->n_hwcs; i++) {
        DESERIALIZE_FIELD(id);
    ...
    // socket->children = (hwc_gs_t**) malloc(socket->n_children * sizeof (hwc_gs_t*));
    socket->children = xmalloc_array(hwc_gs_t*, socket->n_children);
    for (i = 0; i < socket->n_children; i++) {
        // DESERIALIZE_FIELD(id);
        // *((int*) &socket->children[i]) = id;
        // socket->children[i] = (hwc_gs_t*) malloc(sizeof (hwc_gs_t));
        socket->children[i] = xmalloc(hwc_gs_t);
...
void serialize_hwc(hw_context_t* hwc, char** dest) {
	...
	printk("hwc->phy_id: %d, hwc->pcpu_id: %d\n", hwc->phy_id, hwc->pcpu_id);
...
/** Extract the sub-topology of a given one with respect to the given pcpu ids
*/
mctop_t* extract_sub_topology(mctop_t* src, mctop_t* dest, int* hwcs_ids, int nb_hwcs){
    hwc_gs_t *hgs[MAX_HWCS_GRPS_NB], *hwc_gs;
    socket_t *sockets[src->n_sockets], *socket;
    hw_context_t *hwc;
    int i, j, k, n_sockets=0, nr_hwcs_grps=0, flag;
    int *new_to_last_socket_ids;

    if(dest == NULL){
        dest = (mctop_t*) xmalloc(mctop_t);
    }

    dest->n_levels = src->n_levels;
    dest->latencies = xmalloc_array(uint, dest->n_levels);
    for(i=0; i<dest->n_levels; i++){
        dest->latencies[i] = src->latencies[i];
    }

    dest->n_hwcs = nb_hwcs;
    dest->socket_level = src->socket_level;
    dest->is_smt = src->is_smt;
    dest->n_hwcs_per_core = src->n_hwcs_per_core;
    dest->has_mem = src->has_mem;

    for(i=0; i<src->n_sockets; i++){
        sockets[i] = NULL;
    }

	dest->hwcs = xmalloc_array(hw_context_t, nb_hwcs);
	for(i=0; i<nb_hwcs; i++){
        hwc = NULL;
        for(j=0; j<src->n_hwcs; j++){
            if(src->hwcs[j].id == hwcs_ids[i]){
                hwc = &src->hwcs[j];
                break;
            }
        }
        ...
        socket = hwc->socket;
        flag = 0;
        for(j=0; j<src->n_sockets; j++){
            if(sockets[j] != NULL && sockets[j]->id == socket->id){ // If that socket has already been found
                flag = 1;
                break;
            }
        }
        ...
        if(src->n_levels > 2){
            hwc_gs = xmalloc(hwc_gs_t);
            *hwc_gs = *hwc->parent;
            flag = 0;
            for(j=0; j<nr_hwcs_grps; j++){
                if(hgs[j]->id == hwc_gs->id){ // If that hwc_grp has already been found
                    flag = 1;
                    break;
                }
            }
            if(!flag){
                hgs[nr_hwcs_grps++] = hwc_gs;
            }
        }
    }

        
    dest->n_sockets = n_sockets;
    dest->sockets = xmalloc_array(socket_t, dest->n_sockets);
    new_to_last_socket_ids = xmalloc_array(int, dest->n_sockets);
    for(i=0; i<dest->n_sockets; i++){
        dest->sockets[i] = *sockets[i];
        
        for(j=0; j<src->n_sockets; j++){
            if(src->sockets[j].id == dest->sockets[i].id){
                new_to_last_socket_ids[i] = j;
            }
        }
    }

    // Establish pointer references on hwcs
    for(i=0; i<dest->n_hwcs; i++){
        hwc = &dest->hwcs[i];

        // Establish link to socket
        for(j=0; j<dest->n_sockets; j++){
            if(dest->sockets[j].id == hwc->socket->id){
                hwc->socket = &dest->sockets[j];
                break;
            }
        }

        // Establish link to parent
        flag = 0;
        if(hwc->parent != NULL){
            for(j=0; j<nr_hwcs_grps; j++){
                if(hgs[j]->id == hwc->parent->id){
                    hwc->parent = hgs[j];
                    flag = 1;
                    break;
                }
        ...
        // Establish link to next hwc
        if(i != dest->n_hwcs-1){
            hwc->next = &dest->hwcs[i+1];
        } else {
            hwc->next = NULL;
        }
    }

    if(src->has_mem == BANDWIDTH){
        dest->mem_bandwidths_r = xmalloc_array(double, dest->n_sockets);
        dest->mem_bandwidths1_r = xmalloc_array(double, dest->n_sockets);
        dest->mem_bandwidths_w = xmalloc_array(double, dest->n_sockets);
        dest->mem_bandwidths1_w = xmalloc_array(double, dest->n_sockets);

        for(j=0; j<dest->n_sockets; j++){
            dest->mem_bandwidths_r[j] = src->mem_bandwidths_r[new_to_last_socket_ids[j]];
            dest->mem_bandwidths1_r[j] = src->mem_bandwidths1_r[new_to_last_socket_ids[j]];
            dest->mem_bandwidths_w[j] = src->mem_bandwidths_w[new_to_last_socket_ids[j]];
            dest->mem_bandwidths1_w[j] = src->mem_bandwidths1_w[new_to_last_socket_ids[j]];
        }
    }


    // Update the sockets details
    dest->node_to_socket = xmalloc_array(uint, dest->n_sockets);
    for(i=0; i<dest->n_sockets; i++){
        socket = &dest->sockets[i];
        socket->socket = NULL;
        socket->parent = NULL;

        if(i != dest->n_sockets-1){
            socket->next = &dest->sockets[i+1];
        } else {
            socket->next = NULL;
        }

        // Links to descendent hwcs
        socket->n_hwcs = 0;
        for(j=0; j<dest->n_hwcs; j++){
            hwc = &dest->hwcs[j];
            if(hwc->socket->id == socket->id){
                socket->n_hwcs++;
            }
        }
        k = 0;
        ...
        for(j=0; j<dest->n_hwcs; j++){
            hwc = &dest->hwcs[j];
            if(hwc->socket->id == socket->id){
                socket->hwcs[k++] = &dest->hwcs[j];
            }
        }

        socket->n_cores = socket->n_hwcs / dest->n_hwcs_per_core;

        // Links to descendent hwc_groups
        socket->n_children = 0;
        if(dest->n_levels > 2){
            for(j=0; j<nr_hwcs_grps; j++){
                if(hgs[j]->parent && hgs[j]->parent->id == socket->id){
                    socket->n_children++;
                }
            }
            k = 0;
        ...
        socket->topo = dest;
        socket->n_siblings = dest->n_sockets - 1;
        socket->n_nodes = dest->n_sockets;

        // Find the new corresponding local node
        flag = 0;
        for(j=0; j<dest->n_sockets; j++){
            if(new_to_last_socket_ids[j] == socket->local_node){
                socket->local_node = j;
                flag = 1;
                break;
            }
        ...
        socket->mem_latencies = xmalloc_array(uint, dest->n_sockets);
        for(j=0; j<dest->n_sockets; j++){
            socket->mem_latencies[j] = src->sockets[new_to_last_socket_ids[i]].mem_latencies[new_to_last_socket_ids[j]];
        }

        if(dest->has_mem == BANDWIDTH){
            socket->mem_bandwidths_r = xmalloc_array(double, dest->n_sockets);
            socket->mem_bandwidths1_r = xmalloc_array(double, dest->n_sockets);
            socket->mem_bandwidths_w = xmalloc_array(double, dest->n_sockets);
            socket->mem_bandwidths1_w = xmalloc_array(double, dest->n_sockets);

            for(j=0; j<dest->n_sockets; j++){
                socket->mem_bandwidths_r[j] = src->sockets[new_to_last_socket_ids[i]].mem_bandwidths_r[new_to_last_socket_ids[j]];
                socket->mem_bandwidths1_r[j] = src->sockets[new_to_last_socket_ids[i]].mem_bandwidths1_r[new_to_last_socket_ids[j]];
                socket->mem_bandwidths_w[j] = src->sockets[new_to_last_socket_ids[i]].mem_bandwidths_w[new_to_last_socket_ids[j]];
                socket->mem_bandwidths1_w[j] = src->sockets[new_to_last_socket_ids[i]].mem_bandwidths1_w[new_to_last_socket_ids[j]];
            }
        }

        dest->node_to_socket[socket->local_node] = i;
    }


    // Update the intermediate hwc_groups details
    if(dest->n_levels > 2){
        for(i=0; i<nr_hwcs_grps; i++){
            socket = hgs[i];
            ...
            for(j=0; j<dest->n_hwcs; j++){
                hwc = &dest->hwcs[j];
                if((hwc->parent != NULL) && (hwc->parent->id == socket->id)){
                    socket->n_hwcs++;
                }
            }
            k = 0;
            ...
            socket->topo = dest;
            socket->n_siblings = nr_hwcs_grps - 1;
            socket->n_nodes = dest->n_sockets;


            // Links to descendent hwc_groups (corresponding to the hwcs)
            socket->n_children = socket->n_hwcs;
            socket->children = xmalloc_array(hwc_gs_t*, socket->n_children);
            for(j=0; j<socket->n_hwcs; j++){
                hwc = socket->hwcs[j];
                hwc_gs = xmalloc(hwc_gs_t);
                hwc_gs->id = hwc->id;
                hwc_gs->level = hwc->level;
                hwc_gs->type = hwc->type;

                hwc_gs->n_hwcs = 0;
                hwc_gs->hwcs = NULL;
                hwc_gs->n_cores = 0;
                hwc_gs->n_children = 0;
                hwc_gs->children = NULL;
                hwc_gs->parent = socket;
                hwc_gs->socket = socket;
                hwc_gs->topo = dest;
                hwc_gs->local_node = socket->local_node;
                hwc_gs->local_node_wrong = socket->local_node_wrong;
                hwc_gs->n_nodes = socket->n_nodes;

                hwc_gs->mem_latencies = socket->mem_latencies;
                hwc_gs->mem_bandwidths_r = socket->mem_bandwidths_r;
                hwc_gs->mem_bandwidths1_r = socket->mem_bandwidths1_r;
                hwc_gs->mem_bandwidths_w = socket->mem_bandwidths_w;
                hwc_gs->mem_bandwidths1_w = socket->mem_bandwidths1_w;
                hwc_gs->pow_info = socket->pow_info;

                socket->children[j] = hwc_gs;
            }
        }
    } else {
        for(i=0; i<dest->n_sockets; i++){
    ...
    DEBUG_PRINT("extract_sub_topology");

    if(dest == NULL){
        dest = xmalloc(mctop_t);
    }
    ...
    DEBUG_PRINT("End of extract_sub_topology");
...
void free_mctop_references(mctop_t *mctop){
    int i;

    DEBUG_PRINT("free_mctop");

    if(mctop->has_mem >= LATENCY){
        xfree(mctop->latencies);
    }
    xfree(mctop->hwcs);

    for(i=0; i<mctop->n_sockets; i++){
        free_socket_references(mctop->sockets + i);
    }

    xfree(mctop->sockets);
    xfree(mctop->cache->latencies);
    xfree(mctop->cache->sizes_OS);
    xfree(mctop->cache->sizes_estimated);
    xfree(mctop->cache);

    if(mctop->has_mem >= BANDWIDTH){
        xfree(mctop->mem_bandwidths_r);
        xfree(mctop->mem_bandwidths1_r);
        xfree(mctop->mem_bandwidths_w);
        xfree(mctop->mem_bandwidths1_w);
    }

    xfree(mctop->node_to_socket);

    xfree(mctop->pow_info);

    DEBUG_PRINT("End of free_mctop");
}

void free_socket_references(socket_t *socket){
    int i;
    
    DEBUG_PRINT("free_socket");
    ...
    if(socket->n_children){
        for(i=0; i<socket->n_children; i++){
            free_socket_references(socket->children[i]);
        }
        xfree(socket->children);
    }

    DEBUG_PRINT("End of free_socket");
}
...
static void schedule(void)
{
    ...
    /* Added by KevinJio */
    next->last_pcpu = next->current_pcpu;
    next->current_pcpu = cpu;

    if(next->current_pcpu != next->last_pcpu){
	...
	domain = next->domain;
        ...
        #ifdef CONFIG_COMPAT
                shared_info = &domain->shared_info->native;
        #else
                shared_info = &domain->shared_info;
        #endif

        dest = NULL;
        // Modify the guest topology
        if(domain->domain_id != 0 && domain->domain_id < 32767){
            /*int last_socket_id = -1, current_socket_id = -1;
            int idx;*/
		if(0==1) goto end_of_the_addition;

            printk("######### PCPU CHANGED #########\n");
            printk("\tDomain ID\t%d\n", domain->domain_id);
            printk("\tvcpu\t%d changed\n", next->vcpu_id);
            printk("\tlast_pcpu:\t%d\n", next->last_pcpu);
            printk("\tcurrent_pcpu:\t%d\n", next->current_pcpu);


            // Test if the new socket is different from the previous one
            /*for(idx=0; idx<complete_topology.n_hwcs; idx++){
            	if(complete_topology.hwcs[idx].pcpu_id == next->last_pcpu){
		    last_socket_id = complete_topology.hwcs[idx].socket->id;
                }
            	if(complete_topology.hwcs[idx].pcpu_id == next->current_pcpu){
		    current_socket_id = complete_topology.hwcs[idx].socket->id;
                }
            }

	    if(current_socket_id == last_socket_id){
		goto end_of_the_addition;
            }*/


            if(complete_topology.n_levels){
		mctop_t* sub_topology = NULL;
		socket_t* socket;
		int* pcpu_ids;
		int i, j;

                struct page_info *last_page, *range_start_page = NULL;

		// Build an array of the pcpu ids on which the domain is running
		pcpu_ids = xmalloc_array(int, domain->max_vcpus);
		for(i=0; i<domain->max_vcpus; i++){
			pcpu_ids[i] = domain->vcpu[i]->current_pcpu;
		}

		// Extract the corresponding mctop topology
		sub_topology = xmalloc(mctop_t);
		sub_topology = extract_sub_topology(&complete_topology, sub_topology, pcpu_ids, domain->max_vcpus);

		// Complete the vcpu-to-pcpu mapping for this domain
		for(i=0; i<domain->max_vcpus; i++){
			for(j=0; j<sub_topology->n_hwcs; j++){
				if(domain->vcpu[i]->processor == sub_topology->hwcs[j].phy_id){
					sub_topology->hwcs[j].vcpu_id = domain->vcpu[i]->vcpu_id;
					sub_topology->hwcs[j].pcpu_id = domain->vcpu[i]->processor;
					printk("Found corresponding hwc\n");
					printk("domain->vcpu[%d]->processor %d\n", i, domain->vcpu[i]->processor);
					printk("domain->vcpu[%d]->current_pcpu %d\n", i, domain->vcpu[i]->current_pcpu);
					printk("sub_topology->hwcs[%d].phy_id %d\n", j, sub_topology->hwcs[j].phy_id);
					printk("sub_topology->hwcs[%d].vcpu_id %d\n", j, sub_topology->hwcs[j].vcpu_id);
					printk("sub_topology->hwcs[%d].pcpu_id %d\n", j, sub_topology->hwcs[j].pcpu_id);
					break;
				}
			}
		}

		// Complete memory size on each node
		for_each_online_node(i){
			page_num_node[i] = 0;
                ...
                last_page = NULL;
                j = -1; // Last node
		spin_lock(&domain->page_alloc_lock);
		page_list_for_each(page, &domain->page_list){
			i = phys_to_nid((paddr_t)page_to_mfn(page) << PAGE_SHIFT);
			page_num_node[i]++;

			if( (i != j) || (page == (&domain->page_list)->tail) ){
			    if(page == (&domain->page_list)->tail){
			    	last_page = page;
			    }

			    if(j != -1){
			        typeof(*socket->mem_ranges) range_start_addr, range_end_addr;
				int addr_found = 0, idx;
			        socket = sub_topology->sockets + j;

			        range_start_addr = get_gpfn_from_mfn(page_to_mfn(range_start_page)) << PAGE_SHIFT;
			        range_end_addr = ((get_gpfn_from_mfn(page_to_mfn(last_page)) + 1) << PAGE_SHIFT) - 1;

			        // Test if that range already existed
			        for(idx=0; idx<socket->nr_mem_ranges_elts; idx++){
			            if( (socket->mem_ranges[idx] == range_start_addr) && (socket->mem_ranges[idx+1] == range_end_addr) ){
			                addr_found = 1;
			                break;
			            }
			        }
				...
				if( !addr_found ){
					// Add a range from range_start_page to last_page
				        typeof(*socket->mem_ranges) *old_mem_ranges;
				        old_mem_ranges = socket->mem_ranges;
	
				        socket->nr_mem_ranges_elts += 2;
				        socket->mem_ranges = xmalloc_array(typeof(*socket->mem_ranges), socket->nr_mem_ranges_elts);
				        for(idx=0; idx<socket->nr_mem_ranges_elts - 2; idx++){
				            socket->mem_ranges[idx] = old_mem_ranges[idx];
				        }

				        socket->mem_ranges[socket->nr_mem_ranges_elts-2] = range_start_addr;
				        socket->mem_ranges[socket->nr_mem_ranges_elts-1] = range_end_addr;
	
				        xfree(old_mem_ranges);
				}
			    }
			    range_start_page = page;
			}

			last_page = page;
			j = i;
		}
		spin_unlock(&domain->page_alloc_lock);

		for(j=0; j<sub_topology->n_sockets; j++){
			socket = sub_topology->sockets + j;
			socket->mem_size = page_num_node[socket->id % MCTOP_LVL_ID_MULTI] * PAGE_SIZE;
		}

		printk("================ PAGE INFO ADDRESSES ================\n");
		printk("FIRST PAGE\n");
		page = (&domain->page_list)->next;
		printk("	page: %ld\n", (unsigned long) page);
		printk("	page_to_mfn(page): %ld\n", (unsigned long) page_to_mfn(page));
		printk("	page_to_maddr(page): %ld\n", (unsigned long) page_to_maddr(page));
		printk("	page_to_virt(page): %ld\n", (unsigned long) page_to_virt(page));
		printk("	page_to_spage(page): %ld\n", (unsigned long) page_to_spage(page));
		printk("	get_gpfn_from_mfn(page_to_mfn(page)): %ld\n", (unsigned long) get_gpfn_from_mfn(page_to_mfn(page)));
		...
		printk("	get_gpfn_from_mfn(page_to_mfn(page)): %ld\n", (unsigned long) get_gpfn_from_mfn(page_to_mfn(page)));
                ...
                dest = shared_info->topology_array;
                serialize_mctop(sub_topology, &dest);
		//free_mctop_references(sub_topology);
		//xfree(sub_topology);
		//sub_topology = NULL;

                printk("============== %p ==============\n", shared_info->topology_array);
                printk("************** %s ****************\n", shared_info->topology_array);
                printk("++++++++++++++ %c ++++++++++++++++\n", shared_info->topology_array[0]);
                printk("++++++++++++++ %c ++++++++++++++++\n", shared_info->topology_array[1]);
                printk("++++++++++++++ %c ++++++++++++++++\n", shared_info->topology_array[2]);
                printk("++++++++++++++ %c ++++++++++++++++\n", shared_info->topology_array[3]);

                // Inform the guest of the change
                send_guest_vcpu_virq(next, VIRQ_TOPOLOGY);
                printk("#####\tVIRQ_TOPOLOGY sent to vcpu %d\n", next->vcpu_id);
            }
        }
    }
