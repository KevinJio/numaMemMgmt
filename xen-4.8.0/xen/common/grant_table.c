...
#define debug_print(string) printk("##### %s\n", string)
#define copy_from_guest_verify(to, from, n)   do { if(n && (to != NULL)) copy_from_guest(to, from, n); } while(0)

int copy_topology_from_guest(XEN_GUEST_HANDLE_PARAM(void) arg);
int copy_topology_array_from_guest(XEN_GUEST_HANDLE_PARAM(void) arg);
hw_context_t* map_hwc_from_kernelspace(hw_context_t** kernel_hwcs, hw_context_t** hyp_hwcs, int n, hw_context_t* hwc);
socket_t* map_socket_from_kernelspace(socket_t** kernel_sockets, socket_t** hyp_sockets, int n, socket_t* socket);

mctop_t complete_topology;
EXPORT_SYMBOL(complete_topology);
char *topology_begin;
EXPORT_SYMBOL(topology_begin);

long
do_send_topology(XEN_GUEST_HANDLE_PARAM(void) arg){
    int res;
    init_null_ptrs_of_mctop(&complete_topology);

    // res = copy_topology_from_guest(arg);
    res = copy_topology_array_from_guest(arg);

    printk("---------------------------send_topology------------------------------\n");
    printk("---------------------------send_topology------------------------------\n");
    printk("---------------------------send_topology------------------------------\n");
    printk("-------------------------complete_topology.n_levels: %d------------------------\n", complete_topology.n_levels);
    printk("-------------------------complete_topology.socket_level: %d--------------------\n", complete_topology.socket_level);
    printk("-------------------------complete_topology.latencies: %p-----------------------\n", complete_topology.latencies);
    // printk("-------------------------complete_topology.latencies[0]: %d--------------------\n", *(complete_topology.latencies));
    printk("---------------------------send_topology------------------------------\n");
    printk("---------------------------send_topology------------------------------\n");
    printk("---------------------------send_topology------------------------------\n");
...
int copy_topology_array_from_guest(XEN_GUEST_HANDLE_PARAM(void) arg){
    ...
    int i;
    char *temp_array, *begin_ptr;
    temp_array = xmalloc_array(char, TOPOLOGY_ARRAY_SIZE);
    begin_ptr = temp_array;
    topology_begin = temp_array;

    if ( unlikely(copy_from_guest(temp_array, arg, TOPOLOGY_ARRAY_SIZE) != 0) )
    {
        gdprintk(XENLOG_INFO, "Error while copying the topology from guest addres space.\n");
        return -1;
    }

    for(i=0; i<TOPOLOGY_ARRAY_SIZE; i++){
        printk("%o", *(temp_array + i));
    }

    /*if(complete_topology.n_levels){ // The topology has already given to the hypervisor
	free_mctop_references(&complete_topology);
    }*/

    deserialize_mctop(&complete_topology, &temp_array);

    printk("___________________________________________________\n");
    printk("______________OKKKKKKKKKKKKKKKKKK________________\n");
    printk("___________________________________________________\n");
...
int copy_topology_from_guest(XEN_GUEST_HANDLE_PARAM(void) arg){
    int i, j;
    XEN_GUEST_HANDLE_PARAM(void) temp_param;
    hw_context_t* old_hwcs;
    mctop_pow_info_t* old_pow_info;
    socket_t* old_sockets;
    int nb_hwc_grps = 0; // Number of intermediate hw_ctx_grp (without the sockets)
    
    hw_context_t **kernel_hwcs, **hyp_hwcs;
    socket_t **kernel_sockets, **hyp_sockets;

    
    debug_print("Entring the copy_topology_from_guest function");
    if ( unlikely(copy_from_guest(&complete_topology, arg, 1) != 0) )
    {
        gdprintk(XENLOG_INFO, "Error while copying the topology from guest address space.\n");
        return -1;
    }

    // Copy of the latencies per level
    debug_print("Copy of the latencies per level");
    temp_param.p = complete_topology.latencies;
    complete_topology.latencies = xmalloc_array(uint, complete_topology.n_levels);
    if ( copy_from_guest(complete_topology.latencies, temp_param, complete_topology.n_levels) != 0)
    {
        gdprintk(XENLOG_INFO, "Error while copying the latencies per level.\n");
        return -1;
    }

    debug_print("Allocating other vars");
    kernel_hwcs = xmalloc_array(hw_context_t*, complete_topology.n_hwcs);
    hyp_hwcs = xmalloc_array(hw_context_t*, complete_topology.n_hwcs);
    kernel_sockets = xmalloc_array(socket_t*, complete_topology.n_sockets);
    hyp_sockets = xmalloc_array(socket_t*, complete_topology.n_sockets);

    // Copy of all the hardware contexts
    debug_print("Copy of all the hardware contexts");
    old_hwcs = complete_topology.hwcs;
    temp_param.p = old_hwcs;
    complete_topology.hwcs = xmalloc_array(hw_context_t, complete_topology.n_hwcs);
    if(copy_from_guest(complete_topology.hwcs, temp_param, complete_topology.n_hwcs) != 0)
    {
        gdprintk(XENLOG_INFO, "Error while copying the hwcs.\n");
        return -1;
    }


    debug_print("Filling the kernel and hypervisor hwcs arrays");
    for(i=0; i<complete_topology.n_hwcs; i++){
        kernel_hwcs[i] = old_hwcs + i;
        hyp_hwcs[i] = complete_topology.hwcs + i;
    }

    // Copy of the sockets
    debug_print("Copy of the sockets");
    old_sockets = complete_topology.sockets;
    temp_param.p = old_sockets;
    complete_topology.sockets = xmalloc_array(socket_t, complete_topology.n_sockets);
    if(copy_from_guest(complete_topology.sockets, temp_param, complete_topology.n_sockets) != 0){
        gdprintk(XENLOG_INFO, "Error while copying the sockets.\n");
        return -1;
    }

    
    for(i=0; i<complete_topology.n_sockets; i++){
        kernel_sockets[i] = old_sockets + i;
        hyp_sockets[i] = complete_topology.sockets + i;
    }

    // Association of the sockets to the hw_ctx (upward) and the hw_ctx together
    debug_print("Association of the sockets to the hw_ctx (upward) and the hw_ctx together");
    for(i=0; i<complete_topology.n_hwcs; i++){
        hyp_hwcs[i]->socket = map_socket_from_kernelspace(kernel_sockets, hyp_sockets, complete_topology.n_sockets, hyp_hwcs[i]->socket);
        hyp_hwcs[i]->next = map_hwc_from_kernelspace(kernel_hwcs, hyp_hwcs, complete_topology.n_hwcs, hyp_hwcs[i]->next);
    }

    // Association of the hw_ctx to the sockets (downward)
    debug_print("Association of the hw_ctx to the sockets (downward)");
    for(i=0; i<complete_topology.n_sockets; i++){
        hyp_sockets[i]->hwcs = xmalloc_array(hw_context_t*, hyp_sockets[i]->n_hwcs);
        for(j=0; j<complete_topology.sockets[i].n_hwcs; j++){
            hyp_sockets[i]->hwcs[j] = map_hwc_from_kernelspace(kernel_hwcs, hyp_hwcs, complete_topology.n_hwcs, hyp_sockets[i]->hwcs[j]);
        }

        if(complete_topology.sockets[i].n_children){
            nb_hwc_grps += complete_topology.sockets[i].n_children;
        }
    }
    return 0;

    // Copy of the intermediate hw_ctx_grps
    debug_print("Copy of the intermediate hw_ctx_grps");
    if(nb_hwc_grps){
         hwc_gs_t* kernel_hwc_grps[nb_hwc_grps];
         hwc_gs_t* hyp_hwc_grps[nb_hwc_grps];
         // int hwc_grp_index = 0;
    
         debug_print("Looping on the sockets"); 
        /*for(i=0; i<complete_topology.n_sockets; i++){
            if(complete_topology.sockets[i].n_children){
                hwc_gs_t** socket_children = complete_topology.sockets[i].children;
                hwc_gs_t* intermediate_hwc_gs;
                complete_topology.sockets[i].children = xmalloc_array(hwc_gs_t*, complete_topology.sockets[i].n_children);
                
                debug_print("Real copy of the intermediate hw_ctx_grps");
                for(j=0; j<complete_topology.sockets[i].n_children; j++){
                    temp_param.p = socket_children + j;
                    intermediate_hwc_gs = xmalloc(hwc_gs_t);

                    copy_from_guest_verify(intermediate_hwc_gs, temp_param, 1);
                    
                    complete_topology.sockets[i].children[j] = intermediate_hwc_gs;
                    complete_topology.sockets[i].children[j]->socket = &(complete_topology.sockets[i]);

                    hyp_hwc_grps[hwc_grp_index] = complete_topology.sockets[i].children[j];
                    kernel_hwc_grps[hwc_grp_index] = (hwc_gs_t*) temp_param.p;
                    hwc_grp_index++;
                }
            }
        }*/

        debug_print("Association of the hwcs to the hw_ctx_grps");
        for(i=0; i<complete_topology.n_hwcs; i++){
            if(kernel_hwcs[i]->parent){
                hyp_hwcs[i]->parent = map_socket_from_kernelspace(kernel_hwc_grps, hyp_hwc_grps, nb_hwc_grps, hyp_hwcs[i]->parent);
            }
        }
    }

    
    // Copy of the cache info
    debug_print("Copy of the cache info");
    temp_param.p = complete_topology.cache;
    complete_topology.cache = xmalloc(mctop_cache_info_t);
    copy_from_guest_verify(complete_topology.cache, temp_param, 1);
        
    debug_print("Inside cache info");
    printk("complete_topology.cache->n_levels= %d\n", complete_topology.cache->n_levels);
    if(complete_topology.cache && complete_topology.cache->n_levels){
        debug_print("complete_topology.cache->latencies");
        temp_param.p = complete_topology.cache->latencies;
        complete_topology.cache->latencies = xmalloc_array(uint64_t, complete_topology.cache->n_levels);
        copy_from_guest_verify(complete_topology.cache->latencies, temp_param, complete_topology.cache->n_levels);
        
        debug_print("complete_topology.cache->sizes_OS");
        temp_param.p = complete_topology.cache->sizes_OS;
        complete_topology.cache->sizes_OS = xmalloc_array(uint64_t, complete_topology.cache->n_levels);;
        copy_from_guest_verify(complete_topology.cache->sizes_OS, temp_param, complete_topology.cache->n_levels);
        
        debug_print("complete_topology.cache->sizes_estimated");
        temp_param.p = complete_topology.cache->sizes_estimated;
        complete_topology.cache->sizes_estimated = xmalloc_array(uint64_t, complete_topology.cache->n_levels);
        copy_from_guest_verify(complete_topology.cache->sizes_estimated, temp_param, complete_topology.cache->n_levels);
    }

    // Copy of the mem_bandwidths info
    if(complete_topology.has_mem == BANDWIDTH){
        debug_print("Copy of the mem_bandwidths info");
        temp_param.p = complete_topology.mem_bandwidths_r;
        complete_topology.mem_bandwidths_r = xmalloc_array(double, complete_topology.n_sockets);
        copy_from_guest_verify(complete_topology.mem_bandwidths_r, temp_param, complete_topology.n_sockets);
        
        temp_param.p = complete_topology.mem_bandwidths1_r;
        complete_topology.mem_bandwidths1_r = xmalloc_array(double, complete_topology.n_sockets);
        copy_from_guest_verify(complete_topology.mem_bandwidths1_r, temp_param, complete_topology.n_sockets);
        
        temp_param.p = complete_topology.mem_bandwidths_w;
        complete_topology.mem_bandwidths_w = xmalloc_array(double, complete_topology.n_sockets);
        copy_from_guest_verify(complete_topology.mem_bandwidths_w, temp_param, complete_topology.n_sockets);
        
        temp_param.p = complete_topology.mem_bandwidths1_w;
        complete_topology.mem_bandwidths1_w = xmalloc_array(double, complete_topology.n_sockets);
        copy_from_guest_verify(complete_topology.mem_bandwidths1_w, temp_param, complete_topology.n_sockets);
    }

    // Copy of the pow_info
    debug_print("Copy of the pow_info");
    old_pow_info = complete_topology.pow_info;
    temp_param.p = old_pow_info;
    complete_topology.pow_info = xmalloc(mctop_pow_info_t);
    copy_from_guest_verify(complete_topology.pow_info, temp_param, 1);
    
    if(complete_topology.pow_info){
        temp_param.p = complete_topology.pow_info->idle;
        copy_from_guest_verify(complete_topology.pow_info->idle, temp_param, MCTOP_POW_TYPE_NUM);
        temp_param.p = complete_topology.pow_info->first_core;
        copy_from_guest_verify(complete_topology.pow_info->first_core, temp_param, MCTOP_POW_TYPE_NUM);
        temp_param.p = complete_topology.pow_info->second_core;
        copy_from_guest_verify(complete_topology.pow_info->second_core, temp_param, MCTOP_POW_TYPE_NUM);
        temp_param.p = complete_topology.pow_info->second_hwc_core;
        copy_from_guest_verify(complete_topology.pow_info->second_hwc_core, temp_param, MCTOP_POW_TYPE_NUM);
        temp_param.p = complete_topology.pow_info->all_cores;
        copy_from_guest_verify(complete_topology.pow_info->all_cores, temp_param, MCTOP_POW_TYPE_NUM);
        temp_param.p = complete_topology.pow_info->all_hwcs;
        copy_from_guest_verify(complete_topology.pow_info->all_hwcs, temp_param, MCTOP_POW_TYPE_NUM);
    }
...
socket_t* map_socket_from_kernelspace(socket_t** kernel_sockets, socket_t** hyp_sockets, int n, socket_t* socket){
    int i;
    debug_print("map_socket_from_kernelspace");
    for(i=0; i<n; i++){
        if(kernel_sockets[i] == socket){
            return hyp_sockets[i];
        }
    }
...
hw_context_t* map_hwc_from_kernelspace(hw_context_t** kernel_hwcs, hw_context_t** hyp_hwcs, int n, hw_context_t* hwc){
    int i;
    debug_print("map_hwc_from_kernelspace");
    for(i=0; i<n; i++){
        if(kernel_hwcs[i] == hwc){
            return hyp_hwcs[i];
        }
    }
